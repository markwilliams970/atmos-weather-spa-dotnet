...
- `scripts/lab/vm-teardown.ps1` (+ `scripts/lab/run-vm-teardown.sh` to run it remotely over WinRM) — the reverse of this guide: stops and removes the `AtmosWeb` site/app pool, deletes the published files, drops the `AtmosDb` database and `atmos_app` login (with before/after query-based validation, not just "the command didn't error"), removes the firewall rule and `C:\ProgramData\atmos\logs`. Defaults to a dry run; `-Force`/`--force` actually performs it. There's no VM equivalent of `dev-harness-create.sh` yet (the CREATE piece here still means following this guide, or the automated Phase C steps, by hand/via WinRM) — only the DELETE half has been formalized for the VM so far.
- `scripts/lab/dev-harness-create.sh` / `scripts/lab/dev-harness-teardown.sh` — the full create/delete pair for the local Docker dev harness (§ "Local dev SQL Server" in `docs/phase-c-build-environment.md`): the `atmos-sql-dev` container/volume, the applied EF Core migration, and the matching User Secrets entry.
