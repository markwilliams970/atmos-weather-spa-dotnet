...
**Sink:** the built-in console/debug provider, captured through IIS's stdout log redirection (`stdoutLogEnabled` in `web.config`, §17) — sufficient for this app's scale. Recommend **not** adding Serilog or another logging library at this stage (CLAUDE.md §26); flagged as a possible future upgrade only if file-based structured logging becomes a real operational need after deployment.
