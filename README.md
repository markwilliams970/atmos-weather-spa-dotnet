...
|---|---|---|
| **Create/redeploy** | `scripts/lab/deploy-to-vm.sh [--force]` | Publishes `Atmos.Web`, generates the current EF Core migration script, transfers both to the VM, and runs `vm-create.ps1` there: idempotent IIS/Hosting-Bundle checks, the SQL Server persistence layer (`AtmosDb` + a dedicated least-privilege `atmos_app` login/user/grants + the migration), the IIS site/app pool, `web.config`'s environment variables (including the connection-string secret — never committed to a file — and disabling IIS's WebDAV module for the site, which otherwise silently blocks `PUT /api/recent/units`), the structured-log directory, and the firewall rule. Ends with a real end-to-end `GET /healthz` through IIS → Kestrel → SQL Server, not just "the commands didn't error." |
...
Both default to a **dry run** (report current state, change nothing) and require an explicit `--force`/`-Force` to actually act. Both have been validated for real against the actual lab VM repeatedly, including a genuine from-scratch redeploy and a teardown that genuinely dropped and re-verified the database was gone — see `docs/phase-c-build-environment.md`'s "Phase E" section for the real bugs that surfaced doing that (and got fixed) the first time these scripts' create-from-scratch paths actually ran. `scripts/lab/dev-harness-create.sh`/`-teardown.sh` are the equivalent pair for the local Docker SQL Server used in day-to-day development, similarly exercised end-to-end (create → real read/write traffic through it → delete).
